...
def main(page: ft.Page):
    ...
    def on_message(message: Message):
        if message.message_type == "chat_message":
            ...
            chat.controls.append(
                ft.Text(message.text, italic=True, color=ft.colors.WHITE54, size=12)
            )
        page.update()
    ...
    def send_click(e):
        message_text = new_message.value.strip()  # Elimina espacios en blanco al principio y al final
        if message_text:
            page.pubsub.send_all(Message(user=page.session.get('user_name'), text=message_text, message_type="chat_message"))
            new_message.value = ""
            page.update()

    user_name = ft.TextField(label="Ingresa tu nombre")

    def join_click(e):
        if not user_name.value:
            user_name.error_text = "El nombre no puede estar vacío."
            user_name.update()
        else:
